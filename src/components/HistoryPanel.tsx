...
export function HistoryPanel({ history, onSelect, onClear }: HistoryPanelProps) {
  ...
  return (
    <div class="mt-4 border border-base-300 rounded-lg">
      <button
        ...
        onClick={() => setOpen(o => !o)}
      >
        <span class="font-medium">History {history.length > 0 && `(${history.length})`}</span>
      ...
      {open && (
        <div class="border-t border-base-300">
          {history.length === 0 ? (
            <p class="px-4 py-3 text-sm text-base-content/40 italic">No history yet</p>
          ) : (
            <>
              <div class="max-h-48 overflow-y-auto">
                {history.map((entry, i) => (
                  ...
                  >
                    ...
                    <span class="text-xs text-base-content/40 shrink-0">{timeAgo(entry.timestamp)}</span>
                  </button>
                ))}
              </div>
